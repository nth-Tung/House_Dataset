...
huyen = ["binh-chanh", "can-gio", "hoc-mon", "cu-chi", "nha-be"]

for h in huyen:
    house_links = []
    for i in range(1, 62):
        try:
            url = f"https://muaban.net/bat-dong-san/ban-nha-huyen-{h}-ho-chi-minh?page={i}"
            driver.get(url)
            time.sleep(3)
    ...
    df.to_csv(f"Huyen{h}.csv", index=False, encoding='utf-8-sig')
    print(df.head())
